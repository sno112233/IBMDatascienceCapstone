...
def get_pie_chart(entered_site):
    ...
    if entered_site=='ALL':
        df_all = filtered_df.groupby('Launch Site')['class'].sum().reset_index()
        fig = px.pie(df_all, values='class', names='Launch Site',
                     title = 'Success Counts vs Failure Counts by site ')
    else:
        ...
        fig = px.pie (counts, values = 'counts', names='outcome',
                      title =f'Success Counts vs Failure counts for {entered_site}')
...
def get_payload_chart(payload_range, entered_site):
